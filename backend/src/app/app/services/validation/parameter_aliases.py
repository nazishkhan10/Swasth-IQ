...
class ParameterAliasResolver:
    ...
    def resolve(cls, parameter_name: str, existing_code: Optional[str] = None) -> Optional[str]:
        """
        Returns the canonical parameter code.
        If existing_code is already canonical, returns it.
        Otherwise tries alias lookup on the parameter_name.
        Falls back to the existing_code (uppercased) if no alias found.
        """
    ...
    def get_all_aliases(cls, canonical_code: str) -> List[str]:
        """Returns all known name aliases for a canonical code."""
